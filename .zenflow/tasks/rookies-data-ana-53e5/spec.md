...
- Player snapshot fields: name, team, position, overall (OVR), development trait (devTrait). Map devTrait to labels: 3=X-Factor, 2=Superstar, 1=Star, 0=Normal.
- OVR computation: prefer `playerBestOvr`; fallback to `playerSchemeOvr`; default 0 if missing/invalid.
- Elites spotlight: display only X-Factor and Superstar rookies as cards sorted by higher dev, then OVR, then name.
- Team analytics: aggregate per current team (from players CSV) with counts, average OVR, best OVR, and dev distribution; sort by avg OVR desc, then team name.
- Position analytics: aggregate per position with counts, average OVR, and dev distribution; sort by avg OVR desc, count desc, pos name.
- KPIs: totals, average OVR, counts of dev tiers, elites share (XF+SS as percentage of total rookies in class).
...
- Output HTML: self-contained document with simple CSS, no external JS libraries; ensure all template placeholders are resolved.
- CLI parameters:
  - `--year <int>` (required)
...
- Dev trait mapping:
  - `3` => `X-Factor`, `2` => `Superstar`, `1` => `Star`, `0` (or unknown) => `Normal`.
...
- Output HTML contract:
  - Contains KPIs, elites cards, team table, hiddens leaderboard, position table, elite-heavy positions list.
...
4. Optional analytics extensions
   ...
   - Secondary sort toggles for team/position tables (server-side pre-sorted).
...
- Helper scripts to add (Phase 2):
  - `scripts/verify_draft_class_analytics.py`:
    ...
    - Parses generated HTML and asserts:
      - KPI values (Total, Avg OVR, XF, SS, Star, Elites share) match recomputed values (with small float tolerance for Avg and percent).
