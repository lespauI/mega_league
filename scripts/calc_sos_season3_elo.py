#!/usr/bin/env python3
"""
Season 3 Strength of Schedule (SoS) via ELO — scaffolding.

CLI contract:
  python3 scripts/calc_sos_season3_elo.py \
    [--games-csv PATH] [--teams-csv PATH] [--elo-csv PATH] \
    [--season3-start-row N] [--include-home-advantage true|false] \
...
def read_games_season3(games_csv: str, start_row: int) -> List[Dict[str, Any]]:
    """Read Season 3 slice of games from MEGA_games.csv starting near `start_row`.
    ...
    logging.info(
        "Season 3 slice: %d games from %s (seasonIndex=%s inferred from row %d; start_row arg=%d)",
        len(rows),
        games_csv,
        target_season,
...
def write_outputs(rows: List[Dict[str, Any]], out_dir: str) -> None:
    ...
    csv_path = os.path.join(out_dir, "sos", "season3_elo.csv")
    json_path = os.path.join(out_dir, "sos", "season3_elo.json")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Season 3 SoS via ELO (scaffold)")
    ...
    ap.add_argument("--season3-start-row", type=int, default=571, help="Row index to start Season 3 slice (default: 571)")
    ap.add_argument(
...
def main(argv: List[str] | None = None) -> int:
    ...
    logging.info("Starting SoS Season 3 ELO calculation (scaffold)")
    logging.info(
        "Args: games_csv=%s teams_csv=%s elo_csv=%s season3_start_row=%d include_home_advantage=%s hfa_elo_points=%d index_scale=%s out_dir=%s",
        args.games_csv,
        args.teams_csv,
        args.elo_csv,
        args.season3_start_row,
    ...
    # Pipeline (stubbed)
    games_rows = read_games_season3(args.games_csv, args.season3_start_row)
    ...
    try:
        out_sched_dir = os.path.join(args.out_dir, "schedules", "season3")
